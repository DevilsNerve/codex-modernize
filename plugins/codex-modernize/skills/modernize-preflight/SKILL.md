...
description: "Check a legacy system's toolchains, source completeness, build infrastructure, and scope boundary before modernization. Use for readiness checks; report gaps without changing legacy source."
---
...
Invocation: `$codex-modernize:modernize-preflight <system-dir> [target-stack]`. These are prompt arguments, not a shell command.
...
Use any answers already given in the session. Ask only the still-unanswered
questions below, bundling them when the available input tool permits it. The most expensive modernization mistakes are things a
person who knows the system answers in seconds and that cost real money to
discover wrong from the source alone. Accept "don't know" for any of them.
...
`$codex-modernize:modernize-brief` most of all, read them from there. Do not paraphrase
...
**3b — Smoke test, escalating.** Builds can create caches, generated source,
and lockfiles. Inspect the build scripts first and run them in a disposable
copy outside `legacy/`, or with verified external output directories. If the
request is report-only/no-execution, report the untested toolchain instead.
 Identify the compiler/interpreter for the
...
- **Deployment/config descriptors** — JCL for batch COBOL, CICS CSD
  ...
  `$codex-modernize:modernize-map` are guesswork.
...
- **Production telemetry** — is an observability/APM MCP server connected,
  ...
  overlay in `$codex-modernize:modernize-assess` Step 4 and timing annotations in
  `$codex-modernize:modernize-map`.)
...
- **Outbound** — things inside `{system}` that depend on source *outside* it
  ...
  `$codex-modernize:modernize-map` topology and any delta catalog only see what is under
...
- **Inbound** — things *outside* `{system}` that depend on things *inside* it.
  This is the **blast radius**: an in-place migration (`$codex-modernize:modernize-uplift`)
...
every downstream command (`$codex-modernize:modernize-brief` above all) and are worth
...
- `uplift` (same-stack version bump) — needs Check 3 green for the **target**
  ...
  Codex-derived and loses the tool's coverage; note that. (c) Did Check 6
  ...
  inbound-referenced shared nodes and say that `$codex-modernize:modernize-brief` must give
  each one an explicit transition decision as its own line item (Check 6
  lists the options), and that `$codex-modernize:modernize-uplift` Step 1 will not migrate a
