...
description: "Assess legacy code or a portfolio of systems for architecture, complexity, debt, security, and modernization options. Produces evidence-backed assessment artifacts, not implementation changes."
---
...
Invocation: `$codex-modernize:modernize-assess <system-dir> [--show-secrets] | --portfolio <parent-dir>`. These are prompt arguments, not a shell command.
...
**Mode select.** If `the invocation text` starts with `--portfolio`, run **Portfolio
...
Follow [portfolio orchestration](../../references/workflows/portfolio-assess.md).
Enumerate immediate subdirectories using filesystem APIs, not a whitespace-splitting shell pipeline.
Run P1–P3 for every system, with bounded concurrency if supported, then render P4.
A failed survey is an explicit unmeasured row, never a zero-risk system.
...
Use these three specialist passes (parallel only when permitted by the runtime guide):
...
1. Run the runtime guide's `quarantine` helper. Use its returned inventory
   path; it checks ignores, tracking, and history. If `rawAllowed` is false,
   refuse `--show-secrets` and use the returned private directory outside the
   project. Report that location. Create private files with mode 0600.
...
- **Recommended Modernization Pattern** (one of: Rehost / Replatform / Refactor / Rearchitect / Rebuild / Replace — with one-paragraph rationale, and the command it routes to: **Replatform / Refactor-in-place same-stack version bump → `$codex-modernize:modernize-uplift`**; Rearchitect/cross-stack → `$codex-modernize:modernize-transform`; Rebuild → `$codex-modernize:modernize-reimagine`)
