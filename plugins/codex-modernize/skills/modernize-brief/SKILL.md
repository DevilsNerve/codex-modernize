...
description: "Synthesize legacy discovery into a phased modernization brief with architecture, behavior contracts, entry criteria, and approval scope. Use to plan a rewrite, rebuild, or version uplift."
---
...
Invocation: `$codex-modernize:modernize-brief <system-dir> [target-stack]`. These are prompt arguments, not a shell command.
...
Target stack: `{target-stack}` (if blank, recommend one based on the assessment findings).
...
stop — they come from `$codex-modernize:modernize-assess`, `$codex-modernize:modernize-map`, and
`$codex-modernize:modernize-extract-rules` respectively. Run those first.

Two more inputs are conditional:

- **`analysis/{system}/PREFLIGHT.md`** — read it if it exists. It records two
  things nothing else has: the human's answers to `$codex-modernize:modernize-preflight`
...
- **`analysis/{system}/DELTA_CATALOG.md`** — **required** whenever the target
  (`{target-stack}`, or your recommendation) is a newer version of the *same* stack.
  ...
  missing, produce it *before* phasing — run `$codex-modernize:modernize-uplift {system} <source> {target-stack} --catalog-only`, or use the
...
user re-ran this command anyway, refresh the requested brief without repeatedly asking for confirmation; record that no newer inputs were found. Either way, note the
...
time. `$codex-modernize:modernize-uplift` Step 1 re-applies the same rules at execution
...
Name the per-phase execution command: `$codex-modernize:modernize-transform` (cross-stack
module rewrite), `$codex-modernize:modernize-reimagine` (greenfield rebuild), or
`$codex-modernize:modernize-uplift` (same-stack version bump — when the target is a newer
...
and tell the approver they steer execution by editing this file. Keep the brief synchronized with explicit user decisions in the session;
current user instructions take precedence over an older artifact.
...
phase's own first slice. For an uplift, `$codex-modernize:modernize-uplift` Step 5a
...
by `$codex-modernize:modernize-map`), a short narrative table: persona, what happens in
...
Present the completed brief. If the user asked only for planning, stop there.
Implementation requires authorization covering the concrete scope and any
unresolved behavior decisions. Reuse existing explicit authorization when it
covers this plan; otherwise request approval of this reviewable artifact.
Do not invent an approver or a signed approval block. Silence is not approval.
