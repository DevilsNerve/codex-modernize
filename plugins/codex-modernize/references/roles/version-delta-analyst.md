...
1. **Known breaking/behavioral changes** for the version pair (current official documentation
   for the framework's migration guide + whatever official tooling reports — see
...
- **Preserve, don't redesign.** Your fixes are the *smallest change that
  ...
  command (`$codex-modernize:modernize-transform`). Adopt a new idiom only where the old one was
