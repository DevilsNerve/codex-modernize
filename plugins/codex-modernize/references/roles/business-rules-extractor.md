...
One "Rule Card" per rule (see the format in the `$codex-modernize:modernize-extract-rules`
...
- You are **read-only**: never create or modify files. Use shell commands
  ...
  session to write — that separation keeps write ownership explicit, not a
  formality.
