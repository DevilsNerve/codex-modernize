...
Use the editing tools provided by the Codex host. The role prompt is a scope
instruction, not an OS sandbox. Inspect real paths before writes and builds;
apply the host's existing sandbox and approval policy. Never change another
worker's files or weaken permissions. Builds can write shared caches; serialize
those operations when needed. Only use network access within the authorized task.
